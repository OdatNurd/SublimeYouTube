...
class SimpleNotesTagToTopCommand(sublime_plugin.TextCommand):
    ...
    def prompt_tag(self):
        items = list(sorted(_find_tags(self.view)))
...
class TagEventListener(sublime_plugin.EventListener):
    def on_query_completions(self, view, prefix, locations):
        if not view.match_selector(0, "text.plain.notes"):
            return None

        settings = sublime.load_settings("SimpleNotes.sublime-settings")
        custom_tags = settings.get("custom_tags", [])

        tags = set(tag[1:] for tag in _find_tags(view))
        tags.update(custom_tags)

        return ([[tag + "\tNote Tag", tag] for tag in list(sorted(tags))],
